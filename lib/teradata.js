...
function getResults(statement, hasResults, results) {
  var result;

  if (hasResults) {
    result = statement.getResultSetAsync()
      .then(function(resultSet) {
        Promise.promisify(resultSet);
        return resultSet;
      });
  } else {
    result = statement.getUpdateCountAsync()
      .then(function(count){
        return count;
      });
  }
  ...
  return result
    .then(function(rs) {
      ...
      return statement.getMoreResultsAsync()
        .then(function(hasMore) {
          if (hasMore) return getResults(statement, true, results);

          return statement.getUpdateCountAsync()
            .then(function(count) {
              if (count !== -1) return getResults(statement, false, results);
              return results;
            });
        });
    });
...
Teradata.prototype.execute = function(sql) {
  var statement,
    resultSets = [];
  ...
  return createStatement.call(this)
    .then(function(s) {
      ...
      return statement.executeAsync(sql)
        .then(function(hasResults) {
          return getResults(statement, hasResults, []);
        });
    })
    .then(function(results) {
      ...
      _.each(resultSets, function(rs) {
        if (rs.toObjArrayAsync) unprocessed.push(rs.toObjArrayAsync());
        else unprocessed.push(Promise.resolve(rs));
      });

      return Promise.all(unprocessed)
    ...
    .finally(function() {
      return Promise.resolve(statement && statement.closeAsync())
        .finally(function() {
          var closeables = [];
          _.each(resultSets, function(rs) {
            if (rs.closeAsync) closeables.push(rs.closeAsync());
          });
          return Promise.all(closeables);
        });
    });
};
